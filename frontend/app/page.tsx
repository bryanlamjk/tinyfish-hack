		...
		<main className="shell">
			<section className="hero">
				<div className="hero-copy">
					...
					<h1>
						Search once. Watch every ticket site agent move in
						parallel.
					</h1>
					<p className="lede">
						Gemini finds the provider shortlist, TinyFish fans out
						across the web, and the dashboard streams each agent as
						it works.
					</p>
				</div>
				<form className="search-panel" onSubmit={handleSubmit}>
					<label className="field field-large">
						...
						<input
							...
							placeholder="Tokyo museum tickets, Seoul food tours, Rome skip-the-line passes..."
							required
							disabled={isRunning}
						/>
					</label>
					...
					<div className="field-grid">
						...
						<label className="field">
							...
							<select
								...
								disabled={isRunning}
								className={
									form.currency ? "" : "placeholder-select"
								}
							>
			...
			<section className="providers-panel">
				<div className="section-heading">
					...
					<p>
						The grounded Gemini discovery pass picks the sites
						TinyFish will search in parallel.
					</p>
				</div>
				<div className="provider-grid">
					{providers.length === 0 ? (
						<div className="empty-card">
							Provider discovery results will appear here once a
							search starts.
						</div>
					) : (
			...
			<section className="streams-panel">
				<div className="section-heading">
					...
					<p>
						Each card tracks one TinyFish run. If TinyFish exposes a
						browser preview URL, it is embedded here.
					</p>
				</div>
				<div className="stream-grid">
					{agentCards.length === 0 ? (
						<div className="empty-card">
							Start a search to see concurrent TinyFish agents
							appear.
						</div>
					) : (
						agentCards.map((agent) => (
							<article className="agent-card" key={agent.siteId}>
								...
								{agent.error ? (
									<p className="agent-error">{agent.error}</p>
								) : null}
			...
			<section className="results-panel">
				<div className="section-heading">
					...
					<p>
						Combined deal summaries land here once the concurrent
						session finishes.
					</p>
				</div>
