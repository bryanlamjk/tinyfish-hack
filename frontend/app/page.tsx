...
const INITIAL_FORM = {
  destination: "Tokyo",
  category: "museum tickets, food tours, and skip-the-line experiences",
  dateHint: "",
  currency: "USD",
  maxResults: 3,
  providerLimit: 4,
...
export default function HomePage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const source = new EventSource(`${API_BASE_URL}/api/searches/${data.session_id}/events`);
    ...
    source.onerror = () => {
      ...
      setErrorMessage("The live event stream disconnected before the session completed.");
      setSessionStatus("error");
      source.close();
      eventSourceRef.current = null;
    };
    ...
    <main className="shell">
      <section className="hero">
        <div className="hero-copy">
          ...
          <p className="lede">
            Gemini finds the provider shortlist, TinyFish fans out across the web, and the dashboard
        ...
        <form className="search-panel" onSubmit={handleSubmit}>
          <label className="field field-large">
            ...
            <input
              ...
              placeholder="food tours, museum tickets, theme park passes..."
              required
            />
          </label>
          ...
          <div className="field-grid">
            ...
            <label className="field">
              ...
              <input
                value={form.dateHint}
                onChange={(event) => setForm((current) => ({ ...current, dateHint: event.target.value }))}
            ...
            <label className="field">
              ...
              <input
                ...
                onChange={(event) =>
                  setForm((current) => ({ ...current, providerLimit: Number(event.target.value) }))
                }
              />
            </label>
            <label className="field">
              ...
              <input
                ...
                onChange={(event) =>
                  setForm((current) => ({ ...current, maxResults: Number(event.target.value) }))
                }
              />
            </label>
            <label className="field checkbox-field">
              ...
              <input
                ...
                onChange={(event) => setForm((current) => ({ ...current, stealth: event.target.checked }))}
              />
            </label>
          </div>

          <button className="search-button" type="submit">
            {sessionStatus === "running" ? "Scanning providers..." : "Launch concurrent search"}
          </button>

          <div className="status-row">
            <span className={`status-chip status-${sessionStatus}`}>{sessionStatus}</span>
      ...
      <section className="providers-panel">
        <div className="section-heading">
          ...
          <p>The grounded Gemini discovery pass picks the sites TinyFish will search in parallel.</p>
        </div>
        <div className="provider-grid">
          {providers.length === 0 ? (
            <div className="empty-card">Provider discovery results will appear here once a search starts.</div>
          ) : (
      ...
      <section className="streams-panel">
        <div className="section-heading">
          ...
          <p>Each card tracks one TinyFish run. If TinyFish exposes a browser preview URL, it is embedded here.</p>
        </div>
        <div className="stream-grid">
          {agentCards.length === 0 ? (
            <div className="empty-card">Start a search to see concurrent TinyFish agents appear.</div>
          ) : (
            agentCards.map((agent) => (
              <article className="agent-card" key={agent.siteId}>
                ...
                {agent.previewUrl ? (
                  <iframe className="stream-frame" src={agent.previewUrl} title={`${agent.providerName} live view`} />
                ) : (
                  <div className="stream-placeholder">
                    <p>No embeddable stream URL yet.</p>
                ...
                <div className="agent-meta">
                  ...
                  {typeof agent.resultCount === "number" ? <span>{agent.resultCount} results</span> : null}
                </div>
                ...
                <ul className="progress-log">
                  {agent.progress.length === 0 ? (
                    ...
                    agent.progress.map((item, index) => <li key={`${agent.siteId}-${index}`}>{item}</li>)
                  )}
                </ul>
              </article>
            ))
          ...
          <div className="results-shell">
            ...
            <div className="site-results-grid">
              {finalPayload.site_results?.map((site) => (
                <article className="site-result-card" key={`${site.provider_name}-${site.start_url}`}>
                  <div className="agent-header">
                    <div>
                  ...
                  <p className="agent-summary">{site.summary || site.error || "No summary returned."}</p>
                  <div className="result-list">
                    {site.results.length === 0 ? (
                      <p className="muted-copy">No deals returned from this site.</p>
                    ) : (
                      site.results.map((result, index) => (
                        <div className="result-item" key={`${site.provider_name}-${index}`}>
                          <h4>{result.title || "Untitled option"}</h4>
                          <p>
                            {result.price || "n/a"} {result.original_price ? `· was ${result.original_price}` : ""}
                          </p>
                          <p>{result.discount_text || result.short_reason_it_is_a_good_deal || "No extra detail."}</p>
                          {result.booking_url ? (
                            <a href={result.booking_url} target="_blank" rel="noreferrer">
                              booking link
                            </a>
                          ) : null}
                        </div>
